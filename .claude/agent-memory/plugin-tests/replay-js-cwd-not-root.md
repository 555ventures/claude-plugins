...
**Corrected 2026-08-26 (review close, specs/20260826/01).** `--root` is still absent and cwd-as-root still holds. What changed: `--setup` no longer requires `--dir`. It now takes `--spec <path>` and derives the scratch worktree path itself (`<root>/.claude/worktrees/spec-<stem>-<6hex>`, the `merge-back.sh branch-for` stem plus a random suffix), and it refuses a caller `--dir` whose basename opens with `replay` (exit 3). `--dir` remains the manual out-of-repo fallback and still wins verbatim when passed; `--apply`/`--teardown` are unchanged. Tests exercising derived setup pass `--spec` with `{cwd: dir}` and read the printed `dir=` value rather than supplying a path.
