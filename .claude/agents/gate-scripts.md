...
- Zero dependencies: `require('fs'|'path'|'child_process'|'os')` only; `jq` is the only external binary in bash. Adding a package is a hard review finding. `autopilot/**` may import ONLY `@anthropic-ai/claude-agent-sdk`, and only from `autopilot/daemon/sdk.js`; any other non-builtin import anywhere, or an SDK import elsewhere, stays a hard finding.
