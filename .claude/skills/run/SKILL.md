---
name: run
description: "Use when exercising this plugin repo locally to see it working — there is no app to boot; 'running' means the test suite, the workflow codegen check, and the status viewer."
...
- **Full gate**: `node spec/scripts/build-workflows.js --check && npm test`
  (codegen drift check + the whole `node:test` suite; both must pass before any change counts).
