...
Amended by: ADR-0028 (superseded in premise — the gray HTML wireframe, kit, atlas, notes layer, client player and render gate are retired; successor brief 26), ADR-0024 (the WALK state and the `design-critic` critic it dispatched are deleted, and `ledger ask`'s question producer is deleted — every journey is drawn, approved and client-reviewed before product approval, with no critic dispatch and no question pinned)

Phase: P2 · Depends on: 22 · Amends: 22 (the SEED → … → SKIN → REVIEW → APPROVED order ratified by ADR-0006 — via ADR-0008); **amended in place 2026-09-07 via ADR-0010** (KIT before screens, THEME and SIGNOFF out of the driver, WALK and CLIENT before approval); **CLIENT closure amended 2026-09-09 via ADR-0012** (capture at note-raise, session resolve refused, dated waiver); **CLIENT surface amended 2026-09-10 via ADR-0013** (a journey player replaces the grid on the client route, theme picked by the client between WALK and CLIENT, candidate flows retired) · Primary workspaces:
