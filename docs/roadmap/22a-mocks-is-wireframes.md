...
Amended by: ADR-0024 (the WALK state and the `design-critic` critic it dispatched are deleted, and `ledger ask`'s question producer is deleted — every journey is drawn, approved and client-reviewed before product approval, with no critic dispatch and no question pinned)
