...
## Discovery Interview (the intake posture)

Foundation Phase 1 intake is a **structured discovery interview**, not a form — both stages run it
this way. The AI is the interviewer, the user is the client; questions ship as `AskUserQuestion`
...
1. The user's last answer opens one or more dimensions. The command calls **`wf-interview-research`**
   (the light sibling of `wf-foundation` — research agents only, **no proposers, no panel**) with
   `args` = paths/keys/booleans (`stage`, `dimensionKeys`, `briefPath`, `contextPaths`,
   `verifyKeys`). Batch every dimension one answer opens into a single parallel call.
...
3. The command writes each menu to `foundation/interview-research/{dimension}.json` (stamping
...
**Discovery↔Panel bridge (no redundancy).** Split by reversibility: `wf-interview-research`
*elicits* (today's options, so the user picks informed); `wf-foundation` *adjudicates* the
hard-to-reverse forks (the MoA panel). A dimension settled in the woven loop is written
**`constrained`** in `## Open Dimensions` — the existing `runProposers: false` selective-skip
signal — and its `interview-research/*.json` is passed to the panel via `contextPaths` so research
isn't repeated. The panel may attach a `minority_position` to that dimension's ADR but never
reopens it as a `hard_fork`.
