...
- `rules-locked` or `skipped` → **consume, never re-prompt.** The canon already exists:
  ...
  brownfield), and record `foundationStackDescriptor` + `design.rulesManifest` in the config
...
(`foundationStackDescriptor`, `design.rulesManifest`, `designRulesHash`) and leaves the actual
...
5. Report: files written, agents generated (kind → name), config summary, T3 triggers chosen,
   ...
   for `foundation` mode, the design-rules categories found and the stamped `designRulesHash`;
