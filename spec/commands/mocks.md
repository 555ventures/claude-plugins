...
The terminal step, and the one loop that runs across closed sessions: `client open --address <url> [--port <n>]` exposes the running serve in the user's own terminal (§ Mocks: Look and Serve; refused outside CLIENT, without `--address`, against a dead address, or a non-localhost address with no `--port`). Every re-run is a pickup, not a fresh read of chat: run `node {driver} --root .`, read `📥 what the client left`, answer each line with its printed command (`notes address --id <id> --change "<what changed>"` after fixing the screen — plus `--screen <label>` or `--journey <j>` for a project-scope one, `notes address --id <id> --change "<question>"` for a question back), then re-run.
