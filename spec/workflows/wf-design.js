...
- **Mock supremacy on mock-bound surfaces:** when a skeleton carries \`sliceRef\`, the slice is the
  design authority for copy, structure, order, and layout. A taste-grade doctrine habit (dialog
...
function workerPrompt(b) {
  const intent = b.kind === 'foundation'
    ...
    ? `You author one whole **coherence group** in one warm pass by **EXPANDING pre-authored skeletons** — you transcribe design decisions to framework code, you never re-derive taste. The plan is the skeletons.json at ${PLAN_PATH}: read its \`skeletons[]\` entries whose \`componentPath\` / \`storyPath\` appear in your batch \`files\` below. Each entry is the **binding design authority** for one surface.\n\n` +
      ...
      `- **\`decision: "author"\`, \`sliceRef\` PRESENT (mock-bound)** — the slice file is the **binding design authority** for structure, element order, copy, and layout. TRANSCRIBE it: build the element hierarchy from the slice's markup, preserve sibling/child ORDER exactly (button order is a contract, not a suggestion), copy EVERY user-visible string VERBATIM (labels, buttons, placeholders, aria-labels, empty-state prose — never paraphrase, shorten, or "improve" copy; "Send invite" never becomes "Send"), and reproduce the layout primitives the slice declares (grid-template-columns / flex-direction / order). Styling VALUES never come from the slice: resolve each literal / mock var through the skeleton's \`tokenMap\` to a repo token ROLE; a styling value with no \`tokenMap\` entry is a gap → return \`blocked\`, never guess and never bake the literal. Mock SAMPLE DATA (member names, emails, dates in list rows) is not copy: render it from props/interpolation (\`Remove \${member.name}\`, never a hardcoded "Remove Jamie Chen") and carry the mock's sample values VERBATIM into the story fixtures — that keeps the rendered catalog comparable to the mock. A deterministic fidelity gate diffs your output against the mock's strings/order/layout after this pass (it understands interpolation templates; it does NOT accept shortened or paraphrased static copy) — divergence fails the run.\n` +
      `- **\`decision: "author"\`, NO \`sliceRef\` (no-mock)** — the skeleton IS the design: build the \`tree\` structure node-for-node, and on each node emit its \`style\` map as token ROLES on that exact element (fill / border / radius / elevation / padding / gap / color / font are roles — emit each as its token, never a literal). A node or property the skeleton left without a role is a gap → return \`blocked\`, never guess.\n\n` +
