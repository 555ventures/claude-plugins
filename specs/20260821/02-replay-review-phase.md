...
| D10 | JJ ruling (2026-08-21, after the Fable 5 consult on this build's own recorded debts): `.claude/agent-memory/` is **not** a pipeline improvement carrier and must never silently become one. core.md § Feedback Loop records the invariant and why; `spec/commands/review.md`'s CLOSE step gains the operational duty — every `.claude/agent-memory/` file a spec's diff touches is disposed at review close, one stated fate each (carry / correct / delete), the same treatment the deviations sidecar already gets. The rule is stated OPERATIONALLY in review.md rather than cited from doctrine, because `shared-for review` does not serve § Feedback Loop — a citation the executing session never receives is not a contract. Scope is deliberately the disposition step ONLY: no memory-review gate, no write hook, no lint over the directory — those remain unearned under core § Incident Policy (recurrence count 1) and the guard candidate stays pre-registered in the deviations sidecar, unbuilt (AC-20260821-02-10) | This closes a contract contradiction, not an incident: § Feedback Loop already states the pipeline improves "through artifacts — never through anyone's memory" and enumerates carriers that are each derived or review-disposed; agent memory was the one carrier with no disposition step, and this build shipped a false one to prove it |
...
- **AC-20260821-02-10**: WHEN `spec/commands/review.md`'s close step and `spec/doctrine/core.md`
  § Feedback Loop are read THE SYSTEM SHALL state that every `.claude/agent-memory/` file a
  spec's diff touches is disposed at review close with one stated fate each (carry / correct /
  delete), and § Feedback Loop SHALL state that agent memory is not an improvement carrier →
