...
brief: 18
build_base: main
diff_base: 980ffd325b1d7710afd520ae2385dedf88d7cb7b
open_markers: 0
---
...
| tests/build/build-driver.test.js | CREATE | tests | AC-20260901-01-1, AC-20260901-01-2, AC-20260901-01-3, AC-20260901-01-4, AC-20260901-01-5, AC-20260901-01-6, AC-20260901-01-7, AC-20260901-01-8, AC-20260901-01-9, AC-20260901-01-10, AC-20260901-01-11, AC-20260901-01-12, AC-20260901-01-14, AC-20260901-01-18 |
...
Sidecar: <spec minus .md>.build/
  ...
  <spec>.build/gate-cap  (touched at ESCALATE; deleting it re-arms one more round per deletion —
                          a green re-armed round proceeds to COMMIT, a red one re-touches the cap)
...
- **AC-20260901-01-8**: WHEN `--mark repair-applied --continued <n> --spawned <n>` is received for the first, second, or third time THE SYSTEM SHALL re-run the gate and print `COMMIT` or the next `REPAIR` round; WHEN received a fourth time THE SYSTEM SHALL refuse with exit 2, touch `<spec>.build/gate-cap`, and every later bare invocation SHALL print `ESCALATE` with both exits until `gate-cap` is deleted; WHEN `gate-cap` is deleted and a re-armed `repair-applied` mark's gate re-run exits 0 THE SYSTEM SHALL print `COMMIT` and leave `gate-cap` absent; WHEN that re-run instead exits non-zero THE SYSTEM SHALL re-touch `<spec>.build/gate-cap` and return to `ESCALATE` (e.g. three red rounds then a fourth mark → stderr contains `cap`, `--state` prints `ESCALATE`; `gate-cap` deleted and a re-armed mark whose gate now exits 0 → stdout prints `COMMIT`, `<spec>.build/gate-cap` absent on disk; the same sequence with a still-red gate → `<spec>.build/gate-cap` exists again, `--state` prints `ESCALATE`); WHEN the gate child spawned by a `repair-applied` mark terminates without an exit code THE SYSTEM SHALL exit 2, leave `repairs[]` unchanged (no phantom round consumed, no inflation of the D6 row's `workers` sums), and leave the state at `REPAIR` for the same round (e.g. round 2 of 3, a gate child killed by signal before exiting → stderr names the round, `repairs.length` stays 1, a subsequent honest `repair-applied` for round 2 is accepted, not refused as round 4) → `tests/build/build-driver.test.js`
...
- **AC-20260901-01-18**: WHEN `--mark <mark>` is received at a state whose printed step does not name that mark THE SYSTEM SHALL exit 2 naming the current state, run no gate, and leave `build-state.json` and every sidecar file unchanged; a mark the current step does name SHALL CONTINUE TO be admitted (e.g. `--mark integrated` at ESCALATE with a now-green tree and `gate-cap` present → exit 2, stderr contains `ESCALATE`, `--state` still prints `ESCALATE`, `gateRuns` unchanged; four `--mark integrated` re-issues at REPAIR → `gateRuns` stays 1, the bare step still reads `round 1 of 3`; a gate that dies without an exit code after `integrated` was recorded → the state re-derives to `INTEGRATION` and a second `integrated` is accepted) → `tests/build/build-driver.test.js`
...
**Why the state derivation reads the gate exit before `gate-cap` (fixed 2026-09-01, review
finding).** `gate-cap` conflated two questions — "the re-arm budget is spent" and "the state
is `ESCALATE`" — that coincide only while the gate is red. A re-armed `repair-applied` whose
gate genuinely passed still found `gate-cap` present and landed `ESCALATE` instead of `COMMIT`,
and the printed "restart cold" escape would have forced a build that actually passed its gate
onto the D7 resume path, stamping a degraded `redCheck: "skipped-resume"` onto the ledger row
of a build that never needed it — a truth-of-record defect, not a cosmetic one, so it was
fixed rather than waived. The state derivation now reads the last gate run's exit first and
consults `gate-cap` only as a discriminator between `ESCALATE` and `REPAIR` on a red gate; the
re-arm branch re-touches `gate-cap` only when the re-armed gate is still red.

**Why every mark is now guarded by the derived state, not just `committed` (2026-09-01,
review finding).** `handleCommitted()`'s `deriveState() !== 'COMMIT'` check was read as the
precedent for that one mark, not for the mark catalogue in general — and the gap it left was
not an accident of coverage, it tracked exactly which marks had *something* to check locally.
`integrated` verifies no artifact at all, so it was the one mark with nothing standing in for
a state guard, and it was accepted at REPAIR, ESCALATE, and COMMIT alike; the same shape of
"local predicate happens to be trivially true elsewhere" reached `red-attributed` (accepted at
TESTS, pre-recording the judgment before RED_ATTRIBUTION ever printed) and `wave-done`
(accepted at TESTS against pre-created files, skipping the dispatch steps it exists to gate).
A per-handler patch for `integrated` alone would have left the other two, and the next mark
added to the catalogue, exposed to the same class. The fix is one shared admission check at
mark dispatch, run before any handler, using the same pure `deriveState()` the bare
invocation's printed step already comes from — so the printed step and the admissible mark
can never drift apart again.

**Why `repair-applied` now runs its gate before recording the round.** The handler recorded
the repair round first and ran the gate after, so a gate child that died without an exit code
still left the round persisted in `repairs[]` — the honest retry was then refused as a fourth
round, `gate-cap` was touched, and the run parked at ESCALATE after only two repair gates had
actually completed, with `workers.continued`/`spawned` on the D6 row inflated by a round that
never produced a result. That is exactly the "state unchanged" half of D1's exit-2 contract:
a refused or failed step must never leave a side effect behind. The reorder runs the gate
first and records the round only once it has a real exit code, so a dying child costs nothing
but the retry.
