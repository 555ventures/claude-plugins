...
| D7 | The doctrine rule lands in **compressed** form, not the Contracts block's verbatim wording, and § Console Output Style's intro and Close-the-loop bullet are re-wrapped to reclaim the lines: the read-load ratchet (`tests/consistency/read-load.test.js`) is one-way by design and 7 shared lines pushed `/spec:design` and `/spec:init` over their caps. Every promise of the verbatim block survives (queue write before render, `--top`/`--after-spec`/`--after-brief`, the `queued` slot, prose-only is a defect). `spec/entrypoints.json` joins scope: the four new `spec-paths spec-queue` call sites — and core.md's now-unwrapped `spec-paths report-render` mention — must be declared or the manifest gate stays red [no-ac: doctrine prose + generated manifest; enforced by read-load.test.js and entrypoints.test.js] | JJ ruling 2026-09-03: shorten the new rule rather than trim unrelated commands or raise a ceiling that exists to force shrinkage |
