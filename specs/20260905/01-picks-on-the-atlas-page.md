...
brief: n/a
diff_base: 5d36d7618837ae7f80cfd460df09ec82e5a707f7
---
...
| D8 | `spec/entrypoints.json` gains **no** row for `spec/scripts/lib/mocks-picks.js` — the manifest enumerates executables only and `tests/consistency/entrypoints.test.js` (AC-20260820-04-1) pins `manifest.length === scannedExecutables.length` with `lib/` excluded by design; the entrypoints File Plan row is withdrawn and AC-20260905-01-12 loses its entrypoints clause (build ruling 2026-09-05, session auto-pick: no product consequence either way, cheapest to reverse) | A lib module is discoverable through its requiring script's manifest row; a lib row would turn the exhaustive pin red by construction (recorded in the deviations sidecar). |
...
- **AC-20260905-01-12**: WHEN `spec/doctrine/mocks.md` is read THE SYSTEM SHALL show under `## Mocks: Page Notes` a paragraph containing `picks.json`, `Pick this`, `one scope per page`, `notes-scope`, `compare table`, and `mocks-picks.js` (D8: no entrypoints row) → `tests/design-atlas.test.js`
...
**Build deviation, folded 2026-09-05 (one-off).** D6 originally added a `spec/entrypoints.json`
row for `lib/mocks-picks.js`; the manifest enumerates executables only and the exhaustive
entrypoints pin excludes `lib/` by design, so the row turned that pin red by construction. D8
withdrew the row and AC-12 lost its entrypoints clause; a lib module is discoverable through its
requiring script's row.

**Review dispositions (rv_e1f06b694443, 2026-09-05).** Waived: the reconcile leg's two
out-of-plan files (a comment-only host hygiene commit, no code token changed). Rejected: the
single-viewport stacking advisory — with one declared viewport "first declared" and "widest" are
the same, so stacking on selection satisfies D3(c) as written; a first-declared-wins tie-break
would be a new Decision. Waived at the fix cap: one test comment narrating provenance in the
D4 pin, queued for the next spec touching `tests/design-atlas.test.js`.
