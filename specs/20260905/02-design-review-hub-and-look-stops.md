...
| tests/mocks/mocks-driver.test.js | MODIFY | tests | AC-20260905-02-18 — retagged existing mark tests, each preceded by `decideLook`; shared fixtures moved to `mocks-driver-fixtures.js` (45 s per-file budget split, specs/20260903/07) |
| tests/mocks/mocks-driver-fixtures.js | CREATE | tests | D11: `decideLook`/`openLook` helpers + the `advanceTo*` builders shared by the two mocks-driver test files (no AC of its own; no `test(` calls) |
| tests/mocks/mocks-driver-look-stops.test.js | CREATE | tests | AC-20260905-02-9, AC-20260905-02-10, AC-20260905-02-11, AC-20260905-02-12, AC-20260905-02-13, AC-20260905-02-14, AC-20260905-02-15 — the look-stop tests, split from `mocks-driver.test.js` under the 45 s budget |
...
- **AC-20260905-02-19**: WHEN `spec-paths design-hub` runs THE SYSTEM SHALL print one line ending in `spec/scripts/design-hub.js` naming an existing file, and `spec-paths nope` prints its usage line containing `design-hub` on stderr; `spec/entrypoints.json` carries a row for `spec/scripts/design-hub.js` [retired: specs/20260905/04-per-project-look-server.md]
...
**Build and review deviations (folded 2026-09-05, all one-offs).** (1) The pre-existing
`design-atlas.js` entrypoints row dropped its `spec/commands/mocks.md` member: D9 removed every
`design-atlas` literal from that file, so the row was over-broad against the reverse-invocation
check. (2) `design-hub.js`'s header dropped a self-citing `spec-paths design-hub` literal, a bare
IP literal, and a dated ruling so the narration and entrypoints sweeps stay green. (3) AC-10's
illustrative theme candidates are asserted by group shape (`ocean`/`ember` over the fixture's
three composed screens), not by the AC's literal label/path strings, because
`direction-composed` needs three screens per direction. (4) `decideLook` also precedes the
existing refusal-path invocations of the five marks, so AC-18's SHALL CONTINUE TO clause is
exercised with a decided stop present rather than left to check ordering. (5)
`design-look-handoff.test.js`'s single pre-D9 look-stop test was replaced by the two AC-16/AC-17
tests; the three Step 5 catalog tests are untouched. (6) Main was red before this spec touched
anything (the 7.82.1 patch left a dated test comment and a four-entry changelog); the comment was
fixed as chore commit 411b8d9 and `diff_base` corrected to it as the true pre-image, the
changelog trimmed under D10. (7) Review's suite leg tripped the 45 s per-file budget on
`tests/mocks/mocks-driver.test.js` (50 s under load); it was split per specs/20260903/07 into
`mocks-driver-fixtures.js` (shared builders, no tests) and `mocks-driver-look-stops.test.js`
(AC-9..15), no assertion changed — the reconcile leg was waived and the ac-matrix leg rejected
by the user on 2026-09-05, and the File Plan rows above were folded to name both files.
