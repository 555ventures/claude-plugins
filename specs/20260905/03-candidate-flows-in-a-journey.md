...
design: false
breaking: false
depends_on: [specs/20260905/02-design-review-hub-and-look-stops.md, specs/20260905/04-per-project-look-server.md, specs/20260906/02-mocks-ends-at-wireframes.md, specs/20260906/05-gray-states-on-every-wireframe.md]
depended_on_by: []
brief: n/a
---
...
| tests/mocks/mocks-driver.test.js | MODIFY | tests | AC-20260905-03-2, AC-20260905-03-3, AC-20260905-03-4, AC-20260905-03-5, AC-20260905-03-6, AC-20260905-03-7, AC-20260905-03-8, AC-20260905-03-11 — `writeVariants(dir, journey, kebab, labels, opts)` helper writing the three state buttons by default like `writeWireframe` (specs/20260906/05 D4) |
...
--reopen journey:<j>   invalidated now lists: approved, approved(all), variant, drawn   (skinned/reviewed retired by specs/20260906/02)

status.json (schemaVersion 1, additive field)
  "journeys": { "<j>": { "drawn", "approved",
...
**Why copy, not reference.** `journey-drawn`, the notes gate, the matrix
...
**Amended 2026-09-06 (cross-spec check, queued behind brief 22a).** This spec now builds after specs/20260906/02–06: `skinned`/`reviewed` are gone from the journey record and the reopen list (02 D1/D7 — the "two new entries" in AC-11 join `approved, approved(all)`); completeness and the promoted copies run `check --states` (05 D1/D2) and the variants fixture writes the three states (05 D4); `variant-picked` inherits the questions gate through `requireNotesResolved` (03 D4, A5); the `variants:<j>` stop keeps the atlas URL — the review page of 04 is the `journey:<j>` look only (04 D6). `depends_on` gained 02 and 05. Version target 7.84.0 is stale; the build takes the next free minor.
