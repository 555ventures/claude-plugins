...
- **AC-20260906-04-8**: WHEN `stop open journey:onboarding` runs with the server up THE SYSTEM SHALL write the stop's `url` as `http://localhost:<port>/review/onboarding.html#stop-<id>`, probe that page for `id="stop-<id>"`, and print the existing two look lines with that URL; WHEN the page cannot be fetched it SHALL CONTINUE TO exit 3 naming `serve --root`; `stop open theme` SHALL CONTINUE TO write the atlas URL, and `stop open variants:onboarding` (when specs/20260905/03 has landed) SHALL CONTINUE TO write the atlas URL matching `atlas/index\.html#stop-` → `tests/mocks/mocks-driver-look-stops.test.js`
