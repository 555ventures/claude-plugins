...
- **AC-20260906-05-6**: WHEN `--mark journey-approved --journey onboarding` runs on a journey already drawn with every screen declaring the three states, with the capture config written and an approve stop decided, and one screen has since been rewritten via `writeWireframe(dir, 'consent', {states: []})` THE SYSTEM SHALL exit 2 with stderr containing `consent.html: missing state(s) empty, loading, error` and `draw the missing states`, and `journeys.onboarding.approved` stays null → `tests/mocks/mocks-driver.test.js` (post-close amendment, 2026-09-07: D2 promises the refusal at both marks and AC-3 delivers only the `journey-drawn` arm — the `promise-carried-not-delivered` shape, exposed by replay rp_d8fecf9d5bce's caught `silent-fallback` mutation on exactly this arm)
