...
| D12 | Bump `spec/.claude-plugin/plugin.json` to the next free minor — target **7.103.0** (7.98.0 through 7.102.0 were spent on 2026-09-08 by sibling work merged into `main`), resolved to whatever is actually free at build time — with the changelog entry `[no-ac: review's version-bump check is the oracle]` | § Planning version discipline; a shipped sibling spends the number a plan-time target named, so the build resolves the minor rather than replaying the pin. |
...
| spec/.claude-plugin/plugin.json | MODIFY | doctrine | D12: version bump to the next free minor (target 7.103.0, resolved at build time) + changelog entry |
