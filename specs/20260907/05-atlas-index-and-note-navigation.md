...
| spec/.claude-plugin/plugin.json | MODIFY | doctrine | D12: version → 7.99.0 + changelog entry |
| tests/design-atlas.test.js | MODIFY | tests | AC-20260907-05-1, AC-20260907-05-5, AC-20260907-05-7, AC-20260907-05-8, AC-20260907-05-11, AC-20260907-05-14 |
| tests/design-atlas-index.test.js | CREATE | tests | AC-20260907-05-2, AC-20260907-05-3, AC-20260907-05-4, AC-20260907-05-6 |
| tests/mocks/notes-layer-navigation.test.js | CREATE | tests | AC-20260907-05-9, AC-20260907-05-10, AC-20260907-05-12, AC-20260907-05-13 |
...
- **AC-20260907-05-1**: WHEN `buildAtlas` renders a root with at least one journey section THE
...
- **AC-20260907-05-2** `[env: CHROME_BIN]`: WHEN the served atlas is loaded and `session` is
...
- **AC-20260907-05-3** `[env: CHROME_BIN]`: WHEN a `.tocrow` for label `L` is activated THE
...
- **AC-20260907-05-4** `[env: CHROME_BIN]`: WHEN the viewport is narrower than 1200px THE
...
- **AC-20260907-05-5**: WHEN `cmdGallery` renders a candidate gallery THE SYSTEM SHALL CONTINUE
  TO emit no `#toc`, no `#shell` and no `.tocrow` element → `tests/design-atlas.test.js`
- **AC-20260907-05-6** `[env: CHROME_BIN]`: WHEN the served atlas is scrolled so that the last
  ...
  `tests/design-atlas-index.test.js`
- **AC-20260907-05-7**: WHEN `GET /__notes/list?screen=**` is served against a root whose
  ...
  `tests/design-atlas.test.js`
- **AC-20260907-05-8**: WHEN `GET /__notes/list?screen=*` is served against that same root THE
  SYSTEM SHALL CONTINUE TO return only `[N1]`, and `?screen=session-live` only `[N2]` →
  `tests/design-atlas.test.js`
- **AC-20260907-05-9**: WHEN the notes layer renders the project panel for a note with
...
- **AC-20260907-05-10** `[env: CHROME_BIN]`: WHEN that `button.nl-anchor` is activated on the
...
- **AC-20260907-05-11**: WHEN `buildAtlas` renders THE SYSTEM SHALL emit `<div id="nl-notes">`
  as the last element inside `#main` → `tests/design-atlas.test.js`
- **AC-20260907-05-12**: WHEN the notes layer initialises on a page declaring
...
- **AC-20260907-05-13**: WHEN the notes layer renders any note row THE SYSTEM SHALL CONTINUE TO
  ...
  `tests/mocks/notes-layer-navigation.test.js`
- **AC-20260907-05-14**: WHEN `page()`'s emitted stylesheet and `viewer.css` are read THE
...
why D2 fixes the row content at one dot and a label, and why AC-20260907-05-1 pins the row
...
AC-20260907-05-5 is the pin that keeps that true. And the project panel mounts after an
