...
| D11 | Bump `spec/.claude-plugin/plugin.json` to **7.102.0** — `spec/.claude-plugin/plugin.json` already reads 7.98.0 (04 in build), both hardened specs numbered `05` claim 7.99.0, `06` claims 7.100.0 and `07` claims 7.101.0 — with the last-3-versions changelog entry `[no-ac: review's version-bump check is the oracle]` | § Planning version discipline; hardened-but-unbuilt siblings hold the numbers they claim. |
...
| spec/.claude-plugin/plugin.json | MODIFY | doctrine | D11: version → 7.102.0 + changelog entry |
