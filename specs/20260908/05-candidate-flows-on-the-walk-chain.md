...
| spec/scripts/design-atlas.js | MODIFY | scripts | D7: journey section meta line `flow: <k> · rejected: <others>` from `status.json`; header comment updated |
...
| tests/design-atlas.test.js | MODIFY | tests | AC-20260908-05-1, AC-20260908-05-9, AC-20260908-05-12 |
...
- A1 (**the premise spike — the lock gate**): drawing a forked journey 2–3 ways and letting the owner pick blind makes the first draw lose often enough to pay for the feature. Executed 2026-09-08 against the real host `umezawabussan-web` (five seed journeys, all APPROVED under the pre-ADR-0010 chain): for each journey the on-disk structure was re-transcribed gray as one column and two structurally different flows were drawn beside it (segment picker as pages / sheet / one page; checklist gate page / inline / read-first; case page + calendar / expand + slots sheet / readiness questions; form page / inline / two-step; list-then-post / post-first / digest); columns shuffled per journey, key sealed before the look; the owner picked one column per journey. Result: `[NEEDS CLARIFICATION: owner picks outstanding — first-draw losses N of 5; lock only at N ≥ 2]`. **if false** (N < 2): mark this spec `superseded` with `superseded_by: "spike 2026-09-08 — the first draw won ≥4 of 5 blind picks; premise not material"`; nothing is built.
