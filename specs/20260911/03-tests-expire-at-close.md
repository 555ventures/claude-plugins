...
| D7 | `/spec:doctor` check 19 "Expired tests still present" runs `node "$(spec-paths test-expiry)" --root . --all-done` (dry run) and reports the retirable count and files; the remedy is the same command with `--apply`, run only after one `AskUserQuestion` that names the count and the files it will delete — never under `--fix`'s line-item path and never silently (AC-20260911-03-8 `[oracle: gate]`). | Bulk deletion of a host's tests is a product decision; a doctor patch batch of ≤4 cannot carry it. |
| D8 | `spec/bin/spec-paths` gains `test-expiry`; `spec/entrypoints.json` gains rows for `test-expiry.js` (entry points: `spec-review-driver.js`, `doctor.md`) and `lib/invariants.js` (`test-expiry.js`); the plugin version bumps via `node scripts/plugin-bump.js --bump --plugin spec --changelog "<paragraph>"` (AC-20260911-03-8 `[oracle: gate]`). | Same manifest and version discipline as every executable. |
| D9 | Live-repo pin: `test-expiry.js --root . --all-done --json` at HEAD reports `retired: []`, and `--invariants` lists `review-legs.js`, `red-check.js`, `ac-matrix.js`, `spec-build-driver.js` and `lib/driver-io.js` (AC-20260911-03-2, AC-20260911-03-9). | The hand sweep already ran here; a non-empty result at HEAD is a rule drift, and the lib row proves the extension-less-require gap is closed. |
...
| spec/scripts/test-expiry.js | CREATE | scripts | CLI per D3/D4; uses `lib/test-scan.js`, `lib/invariants.js`, `lib/spec-sections.js` (`parseAcBullets`, `normalizeForPinCheck`, `AC_ID_RE_GLOBAL`), `lib/frontmatter.js`, `lib/host-config.js` |
| spec/scripts/spec-review-driver.js | MODIFY | scripts | `doCloseWork()` expiry call + `tests` row key (D5); CLOSE step 🧹 line (D6) |
| spec/commands/doctor.md | MODIFY | doctrine | Check 19 per D7 (fits the 500-line read-load budget: doctor loads 420 today) |
| spec/bin/spec-paths | MODIFY | scripts | Key `test-expiry`; usage line updated (D8) |
| spec/entrypoints.json | MODIFY | other | Rows for the two new files (D8) |
...
```text
node spec/scripts/test-expiry.js --root <r> --spec <spec path>  [--apply] [--json]
node spec/scripts/test-expiry.js --root <r> --all-done          [--apply] [--json]
node spec/scripts/test-expiry.js --root <r> --invariants        [--json]
...
- Doctor: check 19 prints `expired tests present: N in M files` (or `none`); the remedy line is
...
- **AC-20260911-03-2**: WHEN `test-expiry.js --root . --invariants` runs over this repository
...
- **AC-20260911-03-3**: WHEN `test-expiry.js --spec <done spec> --json` (dry run) runs over a
...
- **AC-20260911-03-8** `[oracle: gate]`: WHEN the gate runs THE SYSTEM SHALL resolve
  ...
  point (`spec/commands/doctor.md` check 19 and the review driver), find a `spec/entrypoints.json`
  row for every new file, and pass `scripts/plugin-bump.js --check`
- **AC-20260911-03-9**: WHEN `test-expiry.js --root . --all-done --json` runs over this
  repository at HEAD THE SYSTEM SHALL report `"retired":[]` and `"applied":false` → test in
  tests/expiry/test-expiry.test.js

## Assumptions (escalation triggers)

- A1: The scanner and pin rules are spec 02's `lib/test-scan.js` and the executed spike recorded
...
- A5: `doctor.md` has 80 lines of read-load headroom (420/500 measured) — **if false:** condense
  check 19 to three lines; never raise the budget.
...
- **Tests expire at close.** The driver's close work runs `test-expiry.js --spec <spec> --apply`
  ...
  failing expiry refuses the close. `/spec:doctor` check 19 runs the same rule as a dry run over
