...
- **AC-20260911-04-17**: WHEN the regex-context set is widened per D10 THE SYSTEM SHALL CONTINUE TO read `arr[0]/2`, `x++/2`, `(a+b)/2` and `foo(a)/2` inside `if(...)` as division, and `count-tests.js --root .` over this repository SHALL CONTINUE TO report exactly 1021 cases — the set was corrected, never blanket-widened → reuses tests/ceiling/count-tests.test.js :: AC-20260911-02-11: WHEN count-tests.js --root . runs over this repository at HEAD
