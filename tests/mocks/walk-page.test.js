...
test('AC-20260912-01-1: buildWalkPage renders the exclusions card head as the section\'s first two children, spelling the count and counting every row shown, open or not', () => {
  const seed = oneScreenSeed(['signin'])
  function rows(n, statuses) {
    const out = []
    for (let i = 0; i < n; i++) {
      const status = (statuses && statuses[i]) || 'open'
      out.push(exclRow('E' + i, { claim: 'claim ' + i, note: 'non-goal: claim ' + i, status, rejected: status === 'overridden' ? '-' : null }))
    }
    return out
  }
  const three = rows(3, ['open', 'open', 'confirmed'])
  const html3 = buildWalkPage({ seed, journey: 'onboarding', notes: [], ledger: three, walk: { journeys: {} }, prefix: '' })
  const body3 = exclusionsSection(html3)
  assert.ok(body3, 'AC-1: the exclusions section must render when rows exist: got\n' + html3)
  assert.match(body3, /^\s*<h2 class="wk-excl-h">Three things this product will not do<\/h2>\s*<p class="wk-excl-lead">/,
    'AC-1: the section\'s first two children must be the heading — counting ALL 3 rows including the confirmed one, not only the 2 open ones — and the lead: got\n' + body3)
  const leadMatch = /<p class="wk-excl-lead">([^<]*)<\/p>/.exec(body3)
  assert.ok(leadMatch && leadMatch[1].includes('recorded as not contested when the work is signed off'),
    'AC-1: the lead must warn that an unanswered row is recorded as not contested when the work is signed off: got\n' + body3)

  const html1 = buildWalkPage({ seed, journey: 'onboarding', notes: [], ledger: rows(1, ['open']), walk: { journeys: {} }, prefix: '' })
  assert.match(exclusionsSection(html1) || '', /<h2 class="wk-excl-h">One thing this product will not do<\/h2>/,
    'AC-1: a single row must read "One thing this product will not do", the spelled singular: got\n' + html1)

  const html10 = buildWalkPage({ seed, journey: 'onboarding', notes: [], ledger: rows(10, new Array(10).fill('open')), walk: { journeys: {} }, prefix: '' })
  assert.match(exclusionsSection(html10) || '', /<h2 class="wk-excl-h">10 things this product will not do<\/h2>/,
    'AC-1: ten rows must read "10 things this product will not do" — a digit from 10, never spelled: got\n' + html10)
})

test('AC-20260912-01-2: buildWalkPage renders no wk-excl-h and no wk-excl-lead anywhere on the page when the journey has no exclusion rows', () => {
  const seed = oneScreenSeed(['signin'])
  const html = buildWalkPage({ seed, journey: 'onboarding', notes: [], ledger: [], walk: { journeys: {} }, prefix: '' })
  assert.doesNotMatch(html, /wk-excl-h/, 'AC-2: an empty card must render no wk-excl-h anywhere on the page: got\n' + html)
  assert.doesNotMatch(html, /wk-excl-lead/, 'AC-2: an empty card must render no wk-excl-lead anywhere on the page: got\n' + html)
})

test('AC-20260912-01-3: buildWalkPage renders the provenance sentence and the client\'s quoted words for an answer: row and a withdrawn: row, naming the humanized screen', () => {
  const seed = oneScreenSeed(['invite', 'session-live'])
  const answerNote = { id: 'N014', screen: 'invite', kind: 'question', status: 'resolved', answer: { verdict: 'yes', text: 'No — the front desk sends every invite.', by: 'session', at: NOW } }
  const withdrawnNote = { id: 'N020', screen: 'session-live', by: 'client', status: 'withdrawn', text: "Show the patient's mobile number." }
  const ledger = [
    exclRow('E1', { claim: 'Letting patients invite themselves from the clinic website.', note: 'answer: N014' }),
    exclRow('E2', { claim: 'A second insurer field.', note: 'withdrawn: N020' }),
  ]
  const html = buildWalkPage({ seed, journey: 'onboarding', notes: [answerNote, withdrawnNote], ledger, walk: { journeys: {} }, prefix: '' })
  const articles = exclusionArticles(html)
  const e1 = articles.find((a) => a.includes('data-id="E1"'))
  const e2 = articles.find((a) => a.includes('data-id="E2"'))
  assert.ok(e1 && e2, 'AC-3 setup: both anchored exclusion rows must render as articles: got\n' + html)

  assert.ok(e1.includes('<p class="wk-excl-from">Because you told us, on Invite:</p>'),
    'AC-3: an answer: row must render "Because you told us, on Invite:" with the screen humanized: got\n' + e1)
  assert.ok(e1.includes('<p class="wk-excl-said">No — the front desk sends every invite.</p>'),
    'AC-3: an answer: row must carry a wk-excl-said quoting the note\'s answer text: got\n' + e1)
  assert.ok(e1.indexOf('wk-excl-from') < e1.indexOf('wk-excl-said'),
    'AC-3: the provenance sentence must come before the quoted words: got\n' + e1)

  assert.ok(e2.includes('<p class="wk-excl-from">Because you took back your request on Session live.</p>'),
    'AC-3: a withdrawn: row must render "Because you took back your request on Session live.": got\n' + e2)
  assert.ok(e2.includes("<p class=\"wk-excl-said\">Show the patient's mobile number.</p>"),
    'AC-3: a withdrawn: row must carry a wk-excl-said quoting the note\'s request text: got\n' + e2)
})

test('AC-20260912-01-4: buildWalkPage renders a non-goal: row and a row whose note id resolves to nothing with the claim alone — no wk-excl-from, no wk-excl-said, no id, and no duplicated claim text', () => {
  const seed = oneScreenSeed(['invite'])
  const ledger = [
    exclRow('E1', { claim: 'SMS appointment reminders', note: 'non-goal: SMS appointment reminders' }),
    exclRow('E2', { claim: 'A ghost claim.', note: 'answer: N999' }),
  ]
  const html = buildWalkPage({ seed, journey: 'onboarding', notes: [], ledger, walk: { journeys: {} }, prefix: '' })
  const articles = exclusionArticles(html)
  const e1 = articles.find((a) => a.includes('data-id="E1"'))
  const e2 = articles.find((a) => a.includes('data-id="E2"'))
  assert.ok(e1, 'AC-4 setup: the non-goal: row must render: got\n' + html)
  assert.ok(e2, 'AC-4 setup: a row whose note id resolves to nothing must still render its claim, never be dropped: got\n' + html)

  for (const [label, art] of [['non-goal:', e1], ['unresolvable answer:', e2]]) {
    assert.doesNotMatch(art, /wk-excl-from/, 'AC-4: a ' + label + ' row must render no wk-excl-from: got\n' + art)
    assert.doesNotMatch(art, /wk-excl-said/, 'AC-4: a ' + label + ' row must render no wk-excl-said: got\n' + art)
  }
  assert.doesNotMatch(e2, /N999/, 'AC-4: the unresolvable note id must never render anywhere on the page: got\n' + e2)
  const claimOccurrences = (e1.match(/SMS appointment reminders/g) || []).length
  assert.strictEqual(claimOccurrences, 1,
    'AC-4: the non-goal claim must render exactly once inside its article, never twice (once as claim, once as a repeated provenance sentence): got ' + claimOccurrences + ' in\n' + e1)
})

test('AC-20260912-01-5: buildWalkPage strips a claim\'s literal "not: " prefix into "We won\'t build: …", and renders every other claim verbatim', () => {
  const seed = oneScreenSeed(['invite'])
  const ledger = [
    exclRow('E1', { claim: 'not: a second insurer field', note: 'non-goal: not: a second insurer field' }),
    exclRow('E2', { claim: 'Multi-currency billing', note: 'non-goal: Multi-currency billing' }),
  ]
  const html = buildWalkPage({ seed, journey: 'onboarding', notes: [], ledger, walk: { journeys: {} }, prefix: '' })
  const articles = exclusionArticles(html)
  const e1 = articles.find((a) => a.includes('data-id="E1"'))
  const e2 = articles.find((a) => a.includes('data-id="E2"'))
  assert.ok(e1 && e2, 'AC-5 setup: both rows must render: got\n' + html)
  assert.ok(e1.includes("<p class=\"wk-excl-claim\">We won't build: a second insurer field</p>"),
    'AC-5: the ledger\'s "not: " prefix must render as "We won\'t build: <claim>", the raw grammar never leaking onto the page: got\n' + e1)
  assert.ok(e2.includes('<p class="wk-excl-claim">Multi-currency billing</p>'),
    'AC-5: a claim carrying no "not: " prefix must render verbatim: got\n' + e2)
})

test('AC-20260912-01-6: buildWalkPage gives a confirmed row, an overridden client-needed row, and an overridden retired-source row their own data-verdict and state sentence, and renders no wk-verdicts inside any of them', () => {
  const seed = oneScreenSeed(['invite'])
  const ledger = [
    exclRow('E1', { claim: 'claim one', note: 'non-goal: claim one', status: 'confirmed', rejected: null }),
    exclRow('E2', { claim: 'claim two', note: 'non-goal: claim two', status: 'overridden', rejected: 'client-needed' }),
    exclRow('E3', { claim: 'claim three', note: 'non-goal: claim three', status: 'overridden', rejected: '-' }),
  ]
  const html = buildWalkPage({ seed, journey: 'onboarding', notes: [], ledger, walk: { journeys: {} }, prefix: '' })
  const articles = exclusionArticles(html)
  const cases = [
    ['E1', 'agree', 'You agreed. It stays out.'],
    ['E2', 'needed', "You need this. We'll treat it as a request and let you know here."],
    ['E3', 'dropped', "We've taken this off the list."],
  ]
  for (const [id, verdict, text] of cases) {
    const art = articles.find((a) => a.includes('data-id="' + id + '"'))
    assert.ok(art, 'AC-6 setup: row ' + id + ' must render: got\n' + html)
    assert.match(art, new RegExp('data-verdict="' + verdict + '"'),
      'AC-6: row ' + id + ' must carry data-verdict="' + verdict + '": got\n' + art)
    assert.ok(art.includes('<p class="wk-excl-state">' + text + '</p>'),
      'AC-6: row ' + id + ' must render the state sentence "' + text + '": got\n' + art)
    assert.doesNotMatch(art, /wk-verdicts/, 'AC-6: a non-open row must render no wk-verdicts block: got\n' + art)
  }
})

test('AC-20260912-01-10: spec/templates/mocks/viewer.css carries a rule for each new .wk-excl* class and never combines .wk-excl with wk-req-status in one selector', () => {
  const css = read('spec/templates/mocks/viewer.css')
  for (const sel of ['.wk-excl-h', '.wk-excl-lead', '.wk-excl-from', '.wk-excl-said', '.wk-excl-state', '.wk-excl[data-verdict] .wk-verdicts']) {
    assert.ok(css.includes(sel), 'AC-10: viewer.css must carry a rule for ' + sel + ' — its absence leaves the card unstyled: no match found in viewer.css')
  }
  const selectorLines = css.split('\n').filter((l) => l.includes('{'))
  const collision = selectorLines.find((l) => l.includes('.wk-excl') && l.includes('wk-req-status'))
  assert.strictEqual(collision, undefined,
    'AC-10: no selector may name both .wk-excl and wk-req-status — this card must never restyle, or be restyled by, the unrelated requests grid: got "' + collision + '"')
})
